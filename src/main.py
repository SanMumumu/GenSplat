...
with install_import_hook(
    ...
    from src.misc.swanlab_tools import update_checkpoint_path
...
def train(cfg_dict: DictConfig):
    ...
    os.environ['SWANLAB_LOG_DIR'] = str(output_dir / 'swanlog')
    
    cfg.train.output_path = output_dir
    
    # Set up logging with SwanLab.
    callbacks = []
    if cfg_dict.swanlab.mode != "disabled":
        @rank_zero_only
        def init_swanlab():
            swanlab.init(
                project=cfg_dict.swanlab.project,
                workspace=cfg_dict.swanlab.get('workspace', None),
                experiment_name=f"{cfg_dict.swanlab.name} ({output_dir.parent.name}/{output_dir.name})",
                mode=cfg_dict.swanlab.mode,
                tags=cfg_dict.swanlab.get('tags', None),
                config=OmegaConf.to_container(cfg_dict),
            )
            return SwanLabLogger(project=cfg_dict.swanlab.project,
                                experiment_name=f"{cfg_dict.swanlab.name} ({output_dir.parent.name}/{output_dir.name})")
    ...
    checkpoint_path = update_checkpoint_path(cfg.checkpointing.load, cfg.swanlab)
