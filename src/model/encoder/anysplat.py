...
class EncoderAnySplat(Encoder[EncoderAnySplatCfg]):
    ...
    def __init__(self, cfg: EncoderAnySplatCfg) -> None:
        ...
        if self.distill:
            ...
            # self.distill_aggregator_v3 = VGGT_v3().aggregator.to(torch.bfloat16)
    ...
    def forward(
        ...
        if self.distill:
            ...
            with torch.no_grad():
                ...
                distill_infos["pts_all"] = distill_pts_all     # [B, V, H, W, 3]
                distill_infos["depth_map"] = distill_depth_map # [B, V, H, W, 1]
