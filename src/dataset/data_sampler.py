...
class SimpleBatchSampler(Sampler):
    """
    A simple batch sampler for a single dataset.
    This ensures the set_epoch function is available for compatibility
    with distributed training and proper batching.
    """
    def __init__(
        self, src_dataset_ls, batch_size, num_context_views, world_size=1, rank=0, sampler=None, generator=None
    ):
        self.base_sampler = None
        self.batch_size = batch_size
        self.num_context_views = num_context_views
        ...
        self.src_batch_samplers = [batch_sampler]
        self.raw_batches = [
            list(bs) for bs in self.src_batch_samplers
        ]  # index in original dataset
        self.n_batches = [len(b) for b in self.raw_batches]
        ...
        # sampling probability: decide by dataset length
        self.prob = torch.tensor(self.n_batches) / self.n_total_batch
